...
const moveFrom = (cave, dicSmall, stopSmall) => {
    let count = 0;
    connections[cave].forEach(aConnectedCave => {
        ...
        if (aConnectedCave === 'end') {
            ...
            count += moveFrom(aConnectedCave, localDic, stopSmall);
        } else if (!stopSmall && aConnectedCave !== 'start') {
            count += moveFrom(aConnectedCave, localDic, true);
        }
    })
    return count;
}
console.log(moveFrom('start', { start: true }, false));
